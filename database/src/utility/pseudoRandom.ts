...
const historyPseudoRandomNumber = (
	min: number,
	max: number,
	n: number,
	m: number,
...
function pseudoRandomNumber(
	...
	const randomNumber =
			someMin * someMax +
			(someMax - someMin) * someMax +
			random ** 2 +
			random * random * someMax ** 2,
		text = `${randomNumber}`,
		maxLength = `${max}`.length,
		minLength = `${min}`.length;

	let number = Number(
		text.slice(Math.round(text.length / 2), Math.round(text.length / 2 + maxLength))
	);
	...
	if (chanceNull) {
		chanceBetween(
			...
			pseudoRandomNumber(0, 100, n, m, array, null, null, false, false, false)
		);
	}
	...
	if (yourArr && array && history)
		number = historyPseudoRandomNumber(
			min,
			max,
			n,
			m,
