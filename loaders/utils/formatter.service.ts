...
class Formatter {
	public static RuWords = (
		...
		if (num === 1 || (firstChar === 1 && secondChar != 1)) text += `${stage[0]}`;
		else if (
		...
		else if (firstChar < 5) text += `${stage[1]}`;
		else text += `${stage[2] || stage[1]}`;

		return text;
	};

	public static Color = (text: string, color: Colors) => color + text + Colors.reset;
	...
	public static Comma = (number: string) => `${number}`.replace(".", ",");
