...
const permanentHash = (input = "") => {
	...
	for (let i = 0; i < input.length; i++) first += input.charCodeAt(i).toString(3);

	for (let i = 0; i < first.length; i++) output += first.charCodeAt(i).toString(2);
...
export { checkInput, permanentHash, hash, unHash, letters, numbers, symbols };
